...
const Pages = (props) => {
    ...
    useEffect(() => {
        if (!bLoading){
            ...
            if (WPConfig.wordpress_url.trim() == ""){
                ...
                axios.get(WPConfig.wordpress_url + "/wp-json/wp/v2/pages/?page=" + iCurrentPage + "&per_page=" + iTotalPerPage + "&_embed=true").then(result => {
                    setLoading(false);
    ...
    const processPost = (posts) => {
        return posts.map(post => <Page key={post.id} post={post} />);
    };
